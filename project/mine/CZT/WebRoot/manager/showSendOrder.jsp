...
<script language="javascript" type="text/javascript"
	language="javascript">
		...
		function endOrder(id){
			if(confirm('您确认不配送直接归档?')){
				document.forms[0].action="<%=path%>/manager/OrderManagerAction_endOrder.do"
				$("#id").val(id);
				$("#form").ajaxSubmit(function (responseResult) {
					if(responseResult!=''){
						...
						if(data[0]=='true'){
							...
							document.forms[0].action="<%=path%>/manager/OrderManagerAction_showOrder.do?currentPage=${param.currentPage}&stateQuery=8&typeQuery=showSendOrder";
							checkForm();
						}else{
							alert(data[1]);
						}
...
<body>
	...
	<form action="<%=path%>/manager/OrderManagerAction_showOrder.do"
		...
		<!--  待配送的订单标示 -->
		<input type="hidden" name="stateQuery" value="8" />
		<!--  出库查询页面的跳转 -->
		<input type="hidden" name="typeQuery" value="showSendOrder" />
		<div id="main">
			<div id="table">
				<div id="ptk">
					<div id="tabtop-tt" style="width: 200px	">待配送的单</div>
				</div>
			</div>
					...
					<tbody>
						<c:forEach var="data" items="${pageResult.content}" varStatus="v">
							...
							<tr class="odd" align="center" height="25" >
								...
								<td class="zw-txt">
								<a href="<%=path%>/manager/sendOrder.jsp?id=${data.id }">配送</a>
								<a href="javaScript:endOrder('${data.id}')">直接归档</a>
