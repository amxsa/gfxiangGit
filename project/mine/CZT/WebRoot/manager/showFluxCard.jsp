...
<script language="javascript" type="text/javascript"
	language="javascript">
		...
		function del(id){
			if(confirm('您确认要删除?')){
				document.forms[0].action="<%=path%>/manager/FluxCardManagerAction_deleteFluxCard.do"
				$("#id").val(id);
				$("#form").ajaxSubmit(function (responseResult) {
					if(responseResult!=''){
						...
						if(data[0]=='true'){
							alert(data[1]);
							document.forms[0].action="<%=path%>/manager/FluxCardManagerAction_showFluxCard.do?currentPage=${param.currentPage}";
							checkForm();
						}else{
							alert(data[1]);
						}
...
<body>
	...
	<form action="<%=path%>/manager/FluxCardManagerAction_showFluxCard.do"
		...
		<div id="main">
			<div id="table">
				<div id="ptk">
					<div id="tabtop-tt" style="width: 200px	">流量卡信息查询</div>
				</div>
			</div>
			...
			<div id="main-tab">
				<div id="info-4">
					...
					<li>状态：<select name="stateQuery"><option value="">全部</option>
						<c:forEach var="fluxCardState" items="${fluxCardState}">
							<option value="${fluxCardState.key }" ${requestScope.state==fluxCardState.key?'selected':'' }>${fluxCardState.value }</option>
						</c:forEach>
					</select>&nbsp;&nbsp;&nbsp;&nbsp;</li>
							...
							<tr class="odd" align="center" height="25" >
								...
								<td class="zw-txt">
									<c:if test="${data.state=='I' }">
										<a href="javaScript:del('${data.mobile}')">删除</a>
									</c:if>
								</td>
							</tr>
