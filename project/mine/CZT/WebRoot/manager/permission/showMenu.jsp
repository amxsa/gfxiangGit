...
<body>
<div class="crumb"><span>当前位置：首页 >>角色管理>>菜单管理</span></div>
<div class="content">
<div>
	...
	<div class="content1"style="margin-left:270px;">
		...
		<ul class="list02">
       	...
       	<li><label>是否有效：</label><select name="status" id="status"><option value="Y">是</option><option value="N">否</option></select></li>
		</ul>
...
function initData(){
	$.ajax({
		type: 'GET',
		async: false,
		url: '<%=path%>/permission/MenuAction_loadMenus.do',
		dataType:"json",
...
function save(flag){
	...
	$.ajax({
	  type: 'POST',
	  async: false,
	  url: '<%=path%>/permission/MenuAction_saveMenu.do',
	  data: data,
	  dataType:"json",
	  success: function(data){
