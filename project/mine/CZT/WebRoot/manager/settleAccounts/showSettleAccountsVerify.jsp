...
<script language="javascript" type="text/javascript"
	language="javascript">
		...
		function getSettle(id){
                ...
                document.forms[0].action="<%=path%>/manager/settleAccounts/settleAccountsVerify.jsp?id="+id;
                document.forms[0].submit();
  		}
	</script>
</head>
...
<body>
	...
	<form action="<%=path%>/manager/SettleAccountsManagerAction_showReviewSettleAccounts.do?"
		...
		<div id="main">
			<div id="table">
				<div id="ptk">
					<div id="tabtop-tt" style="width: 200px	">待审核订单</div>
				</div>
			</div>
					...
					<tbody>
						<c:forEach var="data" items="${pageResult.content}" varStatus="v">
							...
							<tr class="odd" align="center" height="25" >
								...
								<td class="zw-txt">
									<c:choose>
										<c:when test="${data.state=='已结算'}">审核通过</c:when>
								...
								<td class="zw-txt">
									<c:choose>
										<c:when test="${data.state=='已结算'}"><a href="<%=path%>/manager/settleAccounts/showVerifyDetail.jsp?id=${data.orderId}" target="_blank">查看详情</a></c:when>
										<c:otherwise>
											
											<a href="javascript:getSettle('${data.orderId }')">审核</a>
										</c:otherwise>
									</c:choose>
								</td>
							</tr>
