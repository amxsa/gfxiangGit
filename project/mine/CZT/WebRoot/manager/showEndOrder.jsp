				...
				$("#form").ajaxSubmit(function (responseResult) {
					if(responseResult!=''){
						...
						if(data[0]=='true'){
							...
							document.forms[0].action="<%=path%>/manager/OrderManagerAction_showOrder.do?currentPage=${param.currentPage}&stateQuery=9&typeQuery=showEndOrder";
							checkForm();
						}else{
							alert(data[1]);
						}
...
<body>
	...
	<form action="<%=path%>/manager/OrderManagerAction_showOrder.do"
		...
		<!--  待出库的订单标示 -->
		<input type="hidden" name="stateQuery" value="9" />
		<!--  出库查询页面的跳转 -->
		<input type="hidden" name="typeQuery" value="showEndOrder" />
		<div id="main">
			<div id="table">
				<div id="ptk">
					<div id="tabtop-tt" style="width: 200px	">待归档的单</div>
				</div>
			</div>
					...
					<tbody>
						<c:forEach var="data" items="${pageResult.content}" varStatus="v">
							...
							<tr class="odd" align="center" height="25" >
								...
								<td class="zw-txt"><a href="javaScript:endOrder('${data.id}')">归档</a>
