package cn.cellcom.czt.action.permission;

import java.io.IOException;
import java.sql.Connection;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;

import org.apache.commons.lang.StringUtils;
import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import cn.cellcom.common.DB.JDBC;
import cn.cellcom.common.data.PrintTool;
import cn.cellcom.czt.po.TDepartment;
import cn.cellcom.web.page.PageBus;
import cn.cellcom.web.page.PageResult;
import cn.cellcom.web.page.PageTool;
import cn.cellcom.web.spring.ApplicationContextTool;
import cn.cellcom.web.struts.Struts2Base;

public class DepartmentAction extends Struts2Base {

	private static final long serialVersionUID = 1L;
	private static final Log log = LogFactory.getLog(DepartmentAction.class);

	public String showDepartment() {
		
		String name = getParameter("nameParam");
		StringBuffer sql = new StringBuffer();
		sql.append("select * from t_department where 1=1  ");
		List<Object> params = new ArrayList<Object>();
		if (StringUtils.isNotBlank(name)) {
			sql.append(" and name  like ? ");
			params.add("%" + name + "%");
			getRequest().setAttribute("nameParam", name);
		}
		PageBus pageBus = new PageBus();
		PageTool page = new PageTool(getParameter("currentPage"), 20,
				getRequest().getRequestURI());
		JDBC jdbc = ApplicationContextTool.getJdbc();
		try {
			PageResult<TDepartment> pageResult = pageBus.queryPageDataMysql(jdbc,
					ApplicationContextTool.getDataSource(), page,
					sql.toString(), params.toArray(), TDepartment.class);
			if (pageResult != null) {
				getRequest().setAttribute("pageResult", pageResult);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

		return "showDepartment";
	}

	public String preAddDepartment() {

		return "addDepartment";
	}

	public String addDepartment() {

		String name = getParameter("name");
		String linkman = getParameter("linkman");
		String telephone = getParameter("telephone");
		String email = getParameter("email");
		
		try {
			 
			JDBC jdbc = ApplicationContextTool.getJdbc();
			
			TDepartment department = jdbc.queryForObject(ApplicationContextTool.getDataSource(), 
						"select * from t_department where name=?", TDepartment.class, new Object[]{name});
				if (department==null) {
					jdbc.update(ApplicationContextTool.getDataSource(),
							"insert into t_department(name,linkman,telephone,email) values(?,?,?,?)",
							new Object[] { name, linkman, telephone,email });
						PrintTool.print(getResponse(), "true|新增数据成功", null);
				}else{
					PrintTool.print(getResponse(), "false|该角色名已存在", null);
				}
				
			} catch (Exception e) {
				// TODO Auto-generated catch block
				try {
					PrintTool.print(getResponse(), "false|未知错误", null);
				} catch (IOException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				e.printStackTrace();
			}
			return null;
		
	}

	/**
	 * 删除角色
	 * @return
	 * @throws IOException
	 */
	public String deleteDepartment() throws IOException {
		String id = getParameter("id");
		if (StringUtils.isNotBlank(id)) {
			StringBuffer sql = new StringBuffer();
			sql.append("delete from t_department  where  id = ?  ");

			JDBC jdbc = ApplicationContextTool.getJdbc();
			Connection conn = null;

			try {
				conn = ApplicationContextTool.getConnection();
				conn.setAutoCommit(false);
				int count = jdbc.update(conn,
						sql.toString(), new Object[] { Integer.parseInt(id) });
				//
				conn.commit();
				conn.setAutoCommit(true);
				if (count > 0) {
					PrintTool.print(getResponse(), "true|删除成功", null);
				} else {
					PrintTool.print(getResponse(), "false|删除失败", null);
				}
				PrintTool.print(getResponse(), "true|删除成功", null);
			} catch (Exception e) {
				try {
					if (conn != null)
						conn.rollback();
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
				log.error("删除联系人失败", e);
				PrintTool.print(getResponse(), "false|删除失败", null);
			} finally {
				jdbc.closeAll();
			}
		} else {
			PrintTool.print(getResponse(), "false|请选择联系人", null);
		}
		return null;
	}

	public String preUpdateDepartment() {
		String id = getParameter("id");
		JDBC jdbc = ApplicationContextTool.getJdbc();
		try {
			if (id != null) {
				TDepartment department = jdbc.queryForObject(
						ApplicationContextTool.getDataSource(),
						"select * from t_department where id=?", TDepartment.class,
						new Object[] { Integer.parseInt(id) });
				getRequest().setAttribute("department", department);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}

		return "updateDepartment";
	}

	public String updateDepartment() {
		String id = getParameter("id");
		String name = getParameter("name");
		String linkman = getParameter("linkman");
		String telephone = getParameter("telephone");
		String email = getParameter("email");
		String oldName = getParameter("oldName");
		
		try {
			JDBC jdbc = ApplicationContextTool.getJdbc();
			if (name.equals(oldName)) {
				jdbc.update(ApplicationContextTool.getDataSource(),
						"update t_department set name=?,linkman=?,telephone=?,email=?  where id=?",
						new Object[] { name, linkman, telephone,email, id });
				PrintTool.print(getResponse(), "true|更新成功", null);
			}else{
				TDepartment department = jdbc.queryForObject(ApplicationContextTool.getDataSource(), 
						"select * from t_department where name=?", TDepartment.class, new Object[]{name});
				if (department!=null) {
					PrintTool.print(getResponse(), "false|已存在该组织名", null);
				}else{
					jdbc.update(ApplicationContextTool.getDataSource(),
							"update t_department set name=?,linkman=?,telephone=?,email=?  where id=?",
							new Object[] { name, linkman, telephone,email, id });
					PrintTool.print(getResponse(), "true|更新成功", null);
				}
			}
		} catch (Exception e1) {
			try {
				PrintTool.print(getResponse(), "false|更新失败", null);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			e1.printStackTrace();
		}
		return null;
		
	}

	public TDepartment getDepartment(Integer id) {
		JDBC jdbc = ApplicationContextTool.getJdbc();
		try {
			if (id != null) {
				TDepartment department = jdbc.queryForObject(
						ApplicationContextTool.getDataSource(),
						"select * from t_department where id=?", TDepartment.class,
						new Object[] { id });
				return department;
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}
}
