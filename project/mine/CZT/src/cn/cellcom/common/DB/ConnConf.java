...
public class ConnConf {
	...
	public ConnConf(Resource resource) {
		try {
			...
			this.driverClassName = DB.getProperty("mysql.driverClassName");
			this.url = DB.getProperty("mysql.url");
			this.user = DB.getProperty("mysql.user");
			this.password = DB.getProperty("mysql.password");
		} catch (IOException e) {
