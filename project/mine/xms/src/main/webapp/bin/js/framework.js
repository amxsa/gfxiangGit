...
jQuery.SelectBox = function(selectobj, options) {
	...
	/**
	 * 在做联动下拉框时，从新选择父级的下拉框，则后面所有的子级下拉框都清空
	 * @author liuqinghui
	 * @date 2016-06-03
	 */
	function clearChildSelect(firstChildId){
		//清空select下面的选项
		$("#" + firstChildId).empty();
		var options = "<option value=''>请选择</option>";
		$("#" + firstChildId).append(options);
		
		//清空  mainCon 块中的ul中的li值
		var $childUL = $("#" + firstChildId).prev().find("ul");
		var liEle = "<li relvalue=''>请选择</li>"
		$childUL.html(liEle);
		
		var $childInput = $("#" + firstChildId).prev().find("input:text");
		var $firstLI = $childUL.find("li").eq(0);
		$childInput.val($firstLI.text());//给下拉框页面上显示的值设为ul中的第一个li的text
		
		$("#" + firstChildId).val("");//将子级下拉框设为""
		
		var childId = $("#" + firstChildId).attr("childId");
		if (childId != undefined){
			clearChildSelect(childId);//递归调用清空子级select
		}
	}
	...
	function loadLater(obj,value){
					...
					$.ajax({
						...
						success: function(xml){
							var child = obj.attr("childId");
							clearChildSelect(child);//清空子级联动下拉框	liuqinghui 2016-06-03
