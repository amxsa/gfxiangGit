...
function getCouponsForOrder(token,totalPrice,commodityInfo,countFlag,sellerId,areaId){
	...
	return {"couponList":canUseList,"unUseList":noCanUseList,"couponsCount":couponsCount}
}
...
/***********以下是超市及商城的优惠券代码***************/
function getCouponsForMultiOrder(token,totalPrice,countFlag,shoppingCartIds,areaId,areaName){
	var canUseList;
	var noCanUseList;
	var couponsCount;
	var time_stamp=Date.parse(new Date());
...
//根据token 总价 商品信息获取可用优惠券数量
function getMultiOrderCouponCount(token,totalPrice,countFlag,shoppingCartIds,areaId,areaName){
	var resqData = getCouponsForMultiOrder(token,totalPrice,countFlag,shoppingCartIds,areaId,areaName);
	return resqData.couponsCount;
}
