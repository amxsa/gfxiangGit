...
/**
 * 图形验证码图片上传sevelet
 * 图片存在在一个独立的不同于其他图片的目录，以便能对此目录经行定时删除
 *
 */
public class ValidateCodePicUploadServlet extends HttpServlet {
	private final static Logger  logger=Logger.getLogger(ValidateCodePicUploadServlet.class);
	private final static SimpleDateFormat sdf=new SimpleDateFormat("yyyyMMdd");
	private static final long serialVersionUID = 1L;
	//图片文件存放目录名
	private static final String DIRETORY="upload"+File.separator+"validateCode";
	//生成缩列图的宽度
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//图片访问路径前缀
		String picUrlPrefix=GlobalStaticValue.upload_server_address+"/validateCode/"+dateStr;
		try {
