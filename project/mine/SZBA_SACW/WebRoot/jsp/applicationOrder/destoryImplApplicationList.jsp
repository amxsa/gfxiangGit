...
<body>
<div class="crumb"><span>当前位置：<a href="<%=path%>/main.jsp" target="main">首页 </a>>> 实施销毁列表</span></div>
<div class="content">
 <div class="toptit"><h1 class="h1tit">按条件查询</h1></div>
<form id="searchForm" action="<%=path%>/applicationOrder/queryForDestory.do?viewName=destoryImplApplicationList" method="post"> 
 ...
 <table width="100%" cellpadding="0" cellspacing="1" border="0" id="table_hover" class="table">
  ...
  <tr>
    ...
    <td>
	    <span class="tdbut">
	    	<c:if test="${aRecord.applicationStatus=='Y'}"><a href="<%=path%>/applicationOrder/queryForPreHandle.do?viewName=destoryImplApplicationPreHandle&taskId=${aRecord.taskId}&applicationId=${aRecord.applicationId}&procInstId=${aRecord.procInstId}">实施销毁</a></c:if>
    		<c:if test="${aRecord.applicationStatus=='S' }"><a href="javascript:queryApplyDetail(${aRecord.applicationId})">查看详情</a> </c:if>
	    </span>
    </td>
  </tr>
  </c:forEach>
