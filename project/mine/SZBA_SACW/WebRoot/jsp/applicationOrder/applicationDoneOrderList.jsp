...
<body>
<div class="crumb"><span>当前位置：<a href="<%=path%>/main.jsp" target="main">首页 </a>>> 我的已办</span></div>
<div class="content">
 <div class="toptit"><h1 class="h1tit">按条件查询</h1></div>
<form id="searchForm" action="<%=path%>/applicationOrder/queryForPage.do?applicationStyle=done" method="post"> 
 <div class="search">
 <div class="sp">
	...
	--%><table width="100%"  cellpadding="0" cellspacing="1" border="0" id="table_hover" class="table">
		...
		<c:forEach var="v" varStatus="sta" items="${data}">
			<tr>
				...
				<td>${v.caseName }</td>
				<td>${mapping:mappingApplicationType(v.procedureCode) }</td>
				<td>${v.accountName }</td>
				<td>${v.departmentName }</td>
				<td>${v.currTaskName }</td>
				<td>${v.preTaskName }</td>
				<td><fmt:formatDate value="${v.applicationCreateTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
				<td>${mapping:mappingApplicationStatus(v.applicationStatus) }</td>
				<td><span class="tdbut">
					    <a href="javascript:showDetail(${v.applicationId })">查看详情</a> 
					</span>
				</td>
			</tr>
		</c:forEach>
...
<script type="text/javascript">
	...
	function showDetail(id){
		$("#searchForm").attr("action", "<%=path %>/applicationOrder/queryById.do?applicationId="+id);
		$("#searchForm").submit();
	}
</script>
</body>
