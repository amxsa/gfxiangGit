...
<body>
<div class="crumb"><span>当前位置：<a href="<%=path%>/main.jsp" target="main">首页 </a>>> 销毁审核列表</span></div>
<div class="content">
 <div class="toptit"><h1 class="h1tit">按条件查询</h1></div>
<form id="searchForm" action="<%=path%>/applicationOrder/queryForDestory.do?viewName=destoryApplicationList" method="post"> 
 ...
 <table width="100%" cellpadding="0" cellspacing="1" border="0" id="table_hover" class="table">
  ...
  <tr>
    ...
    <td>${mapping:mappingDepartmentNameById(aRecord.leadDepartmentId)}</td>
    <td>${mapping:mappingDepartmentNameById(aRecord.assistDepartmentId)}</td>
    <td>
    	${mapping:mappingApplicationStatus(aRecord.applicationStatus) }
    </td>
    <!-- 查看或者入库的已知参数分别是：任务ID,财物ID,申请单ID,流程实例ID -->
    <td><span class="tdbut">
    <c:if test="${aRecord.hisStatus=='1'}"><a href="<%=path%>/applicationOrder/queryForPreHandle.do?viewName=destoryApplicationPreHandle&taskId=${aRecord.taskId}&applicationId=${aRecord.applicationId}&procInstId=${aRecord.procInstId}">审核</a></c:if>
