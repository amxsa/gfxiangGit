...
<body>
<div class="crumb"><span>当前位置：<a href="<%=path%>/main.jsp" target="main">首页 </a>>> 入库审核列表</span></div>
<div class="content">
 <div class="toptit"><h1 class="h1tit">按条件查询</h1></div>
<form id="searchForm" action="<%=path%>/applicationOrder/queryForIn.do" method="post"> 
 <div class="search">
 	<div class="sp">
  ...
  <p><label>申请时间</label>
        <input name="condiStartTime" type="text"  value="<fmt:formatDate value='${condition.condiStartTime }' pattern='yyyy-MM-dd'/>" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="width:140px;"/>
    <i>至</i>
        <input name="condiEndTime" type="text"  value="<fmt:formatDate value='${condition.condiEndTime }' pattern='yyyy-MM-dd'/>" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="width:140px;"/>
    </p>
   </div>
  ...
  <div class="toptit">
    <h1 class="h1tit">申请单列表</h1>
    <div class="but1"><input name="" type="button" value="批量审核" class="batchApplyPreHandle"/></div>
  </div>
 <table width="100%" cellpadding="0" cellspacing="1" border="0" id="table_hover" class="table">
  ...
  <tr>
  	<td><c:if test="${aRecord.hisStatus=='1'}"><input type="checkbox" name="proId" value="${pro.id}" class="checkBoxPreRow"/></c:if></td>
    ...
    <input type="hidden" name="applicationId2" value="${aRecord.applicationId}"/>
    <input type="hidden" name="taskId2" value="${aRecord.taskId}"/>
    <input type="hidden" name="procInstId2" value="${aRecord.procInstId}"/>
    <td><span class="tdbut">
    <c:if test="${aRecord.hisStatus=='1'}"><a href="javascript:void(0)" onclick="doApply('intoApplicationPreHandle', '${aRecord.taskId}', '${aRecord.applicationId}', '${aRecord.procInstId}')">审核</a></c:if>
