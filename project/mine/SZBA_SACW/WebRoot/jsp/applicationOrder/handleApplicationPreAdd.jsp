...
<c:if test="${fn:length(detail.excessPros) > 0}">
	<c:redirect url="/property/queryForPage.do?viewName=applyHandlePropertyList" />
</c:if>
<div class="crumb"><span>当前位置：<a href="<%=path%>/main.jsp" target="main">首页 </a>>> 财物处置申请</span></div>
<div class="content">
<div class="detal"><h2 class="h4tit">财物处置申请</h2></div>
 ...
 <table width="100%" cellpadding="0" cellspacing="1" border="0" class="table">
  ...
  <tr>
	...
	<td>
		<select name="disposalKey">
			${mapping:generateDisposalKeySelect(p.actualPosition) }
		</select>
	</td>
	<%--<td>
		<textarea name="disposalReason"></textarea>
	</td>
	--%><td>
		${p.proRemark}
	</td>
	...
	<input type="hidden" value="{'disposal':{'code':'', 'description':''},'procedure':{'code':'PROCEDURE003', 'description':'财物处置流程'}}" name="dpResultStr"/>
 </table>
...
$(document).ready(function(){
	...
	$('#selTemp').on('click',function(){
		var path=sysPath+'/templet/queryTemplet.do?contentType=38';
		art.dialog.data('contentType',38);  //处置申请呈批表模版 38
...
function sendData(){
	...
	$(proTr).each(function(i){
		...
		var disposalKey = $(this).find("select[name='disposalKey']").val();
		var disposalReason = $(this).find("textarea[name='disposalReason']").val();
		var proName = $(this).find("input[name='proName']").val();
		var proNo = $(this).find("input[name='proNo']").val();
		
		if(proId != null && proId != ''){
			proIds += "{'proId':'"+proId+"', 'proStatus':'"+proStatus+"', 'proType':'"+proType+"', 'actualPosition':'"+actualPosition+"', 'disposalKey':'"+disposalKey+"', 'disposalReason':'"+disposalReason+"', 'proName':'"+proName+"', 'proNo':'"+proNo+"'},";  //此处将财物状态带上，是为了在财物调用和处置流程时作为一个判断标识
		}
	});
	...
	$.ajax({
		...
		success:function(backData){
			if(backData.state == 'Y' || backData == null){
				alert(backData.msg);
				enableBtn("#submitBtn", "查看详情", 'showHandleDetail('+backData.obj.applicationId+')', ue);
				//window.location.href = sysPath + "/property/queryForPage.do?viewName=applyHandlePropertyList";
