...
<body>
<div class="crumb">
	<span>当前位置：<a href="<%=path%>/main.jsp" target="main">首页 </a>>> 笔录接收列表</span>
</div>
<div class="content">
...
var recordReceive={
	...
	bindEvent:function(){
		$('#searchData').on('click',function(){
			recordReceive.search();
		});
	},
		
	search:function(){
		var localUrl=sysPath+'/client/clientRequestMap.do';
		var clientUrl='/transfer/receiveList.do';
		var params={
			...
			if(data.state=='Y'){
				...
				if(records.length!=0){
					for(var i=0;i<records.length;i++){
						...
						var op='';
						if(obj.transferStatus=='YJZ'){
							op='<a href="javascript:recordReceive.receiveData(\''+obj.transferID+'\',\''+obj.type+'\')" style="margin-left:5px;">接收</a>'
							+'<a href="javascript:recordReceive.refuseData(\''+obj.transferID+'\',\''+obj.type+'\')" style="margin-left:5px;">拒绝</a>';
						}
						tr=tr+'<tr><td>'+createTime+'</td>'
						+'<td>'+obj.recorder+'</td>'
						+'<td>'+obj.transfereeName+'</td>'
						+'<td>'+recordReceive.getType(obj.type)+'</td>' 
						+'<td>'+recordReceive.getStatus(obj.transferStatus)+'</td>'
						+'<td>'
						+'<span class="tdbut">'
						+'<a href="javascript:recordReceive.lookData(\''+createTime+'\',\''+obj.recorder+'\',\''+obj.transferID+'\',\''+obj.type+'\')">查看</a>'
						+ op
						+'</span></td></tr>';
					}
				}else{
					tr='<tr><td colspan="6" style="color:red;">暂无接收记录</td></tr>';
				}
			}else{
	...
	lookData:function(createTime,recorder,recordID,type){
		...
		var cb=function(data){
			if(data.state=='Y'){
				...
				for(var i in transfer){
					var text=transfer[i];
					if(i=='transferStatus'){
						text=recordReceive.getStatus(text);
					}else if(i=='handleDate'&&text!=''&&text!=null){
		...
		$('#xqTab td[name="type"]').html(recordReceive.getType(type));
		clientAjaxPost(localUrl,clientUrl,params,cb);
		art.dialog({
	...
	//接收
	receiveData:function(recordID,type){
		var flag = window.confirm("确定接收记录？");
		if(flag){
			var localUrl=sysPath+'/client/clientRequestMap.do';
			var clientUrl='/transfer/receive.do';
			var params={
				'transferID':recordID,
				'type':type
			}
	...
	//拒绝
	refuseData:function(recordID,type){
		var flag = window.confirm("确定拒绝记录？");
		if(flag){
			art.dialog({
				content:$('#refuseInfoDiv').html(),
			    title: '拒绝原因',
			    width: 300,
			    top:100,
			    ok: function () {
			    	...
			    	var localUrl=sysPath+'/client/clientRequestMap.do';
					var clientUrl='/transfer/refuse.do';
					var params={
						'transferID':recordID,
						'type':type,
						'reason':reason
