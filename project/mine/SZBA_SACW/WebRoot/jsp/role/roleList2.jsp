...
<div class="toptit">
	    ...
	    <div class="but1"><input name="" type="button" value="给用户分配对应角色"  onclick="batchAllot()"/></div>
</div>
<form id="iconForm" action=""  method="post">
	<!-- 上一页面 提交过来的iconid -->
	<input type="hidden" name="iconids" value="${iconIds}" />
	<input type="hidden" name="roleList" value="${roles}" />
	<table width="100%" cellpadding="0" cellspacing="1" border="0" id="table_hover" class="table">
		...
		<c:forEach var="v" items="${data}">
			<tr>
			    <td >
			    	<input type="checkbox" id="role" name="roles" value="${v.id }"/>
			    </td>
				<td>${v.name }</td>
				<td>${v.description }</td>
				<td>${v.priority }</td>
    ...
    function batchAllot(){
	  ...
	  if(flag == false){
	     ...
	     var f=document.getElementById('iconForm');
	      f.action="<%=request.getContextPath()%>/role/allotAccountRole.do?account=${account}";
	      f.submit();
}

$(document).ready(function() {
	//已选角色回显 
	var roles=$('#iconForm input[name="roleList"]').val();
	if(roles!=null&&roles!=''){
		var roleArr=roles.split(';');
		$('#iconForm input[name="roles"]').each(function(idx,el){
			var ri=$(el).val();
		...
		if(message != ''){
			if(message=='success'){
				alert('添加成功！');
				window.location.href="<%=path%>/account/queryForPage.do?account=${account}";
