...
<body>
<div class="crumb"><span>当前位置：<a href="<%=path%>/main.jsp" target="main">首页 </a>>> 财物出库</span></div>
	...
	<table width="100%" cellpadding="0" cellspacing="1" border="0"
		...
		<c:forEach items="${detail.properties }" var="p" varStatus="sta">
			<tr class="proTr">
				...
				<td class="rfidNum">222</td>
				<td class="warehouseId">库位1</td>
...
function sendData(){
	...
	flag = false;
	$('.proTr').each(function(){
		proId=$(this).find(":hidden[name='proId']").val();
	...
	$.ajax({
		...
		success:function(backData){
			if(backData.state === "Y"){
				alert("已审批完成");
				window.location.href=sysPath+"/applicationOrder/queryForOut.do";
			}else{
				alert(backData.msg);
