...
<body>
<div class="crumb"><span>当前位置：<a href="<%=path%>/main.jsp" target="main">首页 </a>>> 调用申请列表</span></div>
<form id="searchForm" action="<%=path%>/property/queryForPage.do?viewName=applyInvokeOutPropertyList" method="post">
<div class="content">
  ...
  <tr> 
   ...
   <c:forEach items="${properties}" var="pro" varStatus="sta">
	  <tr>
	    ...
	    <td> 
	    	<span class="tdbut">
		    <a href="javascript:void(0)" onclick="doQueryApply(this, '${pro.id}')">申请调用</a>
		    </span>
		</td>
	  </tr>
  </c:forEach>
...
<script type="text/javascript">
	...
	function doQueryBatchApply(){
		...
		$(checkdeTrs).each(function(){
			temp = $(this).find(":hidden[name='actualPosition']").val();
			if(actualPosition !== "" && actualPosition !== temp){
				alter("调用财物需在相同的仓库");
				flag = false;
				return false;
			}
			
			acturalPosition = temp;
		});
		...
		$("#searchForm").attr("action", "<%=path%>/applicationOrder/queryForBatchApply.do?viewName=invokeOutApplicationPreAdd" );
	...
	function doQueryApply(mythis, proId){
		...
		$("#searchForm").attr("action", "<%=path%>/applicationOrder/queryForBatchApply.do?viewName=invokeOutApplicationPreAdd&proId="+ proId);
		searchPro();
	}
