...
<body>
<div class="crumb"><span>当前位置：<a href="<%=path%>/main.jsp" target="main">首页 </a>>> 调用归还列表</span></div>
<form id="searchForm" action="<%=path%>/property/queryForPage.do?viewName=applyInvokeIntoPropertyList" method="post">
<div class="content">
  ...
  <tr> 
   ...
   <c:forEach items="${properties}" var="pro" varStatus="sta">
	  <tr>
	    ...
	    <td> 
	    	<span class="tdbut">
		    <a href="javascript:void(0)" onclick="doQueryApply(this, '${pro.id}')">申请归还</a>
		    </span>
		</td>
	  </tr>
  </c:forEach>
...
<script type="text/javascript">
	...
	function doQueryBatchApply(){
		...
		$(checkdeTrs).each(function(){
			temp = $(this).find(":hidden[name='status']").val();
		...
		$("#searchForm").attr("action", "<%=path%>/applicationOrder/queryForBatchApply.do?viewName=invokeIntoApplicationPreAdd" );
	...
	function doQueryApply(mythis, proId){
		...
		$("#searchForm").attr("action", "<%=path%>/applicationOrder/queryForBatchApply.do?viewName=invokeIntoApplicationPreAdd&proId="+ proId);
		searchPro();
	}
