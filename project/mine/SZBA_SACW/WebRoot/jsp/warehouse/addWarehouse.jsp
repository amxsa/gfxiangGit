...
<div class="content">
<form id="checkForm" action="<%=path %>/warehouse/addWarehouse.do" method="post">
 <div class="toptit"><h1 class="h1tit">库位信息</h1></div>
 <ul class="list02a w50">
 	<li><label><span style="color:red;">*</span>库位编号</label><input name="serialNumber" type="text" value="" /></li>
	<li><label><span style="color:red;">*</span>库位名称</label><input name="name" type="text" value="" /></li>
	...
	<%if(isXTGLY==true){ %>
		<div class="department-box model-select-box">
			...
			<input class="department-input" type="hidden" name="departmentID" value="<%=departmentId%>"></input>
		</div>
	<%} else{ %>
	...
	<li><label>库位类别</label>
 	...
 	<li><label>体积大小</label><input name="volume" type="text" value="" /></li>
 	<li><label>存放数量</label><input name="grossQuantity" type="text" value="" /></li>
 	<li><label>已放数量</label><input name="existQuantity" type="text" value="" /></li>
 	<li><label>楼号</label><input name="building" type="text" value="" /></li>
 	<li><label>楼层</label><input name="floor" type="text" value="" /></li>
	<li><label>房间号</label><input name="room" type="text" value="" /></li>
	<li><label>柜号</label><input name="cabinet" type="text" value="" /></li>
	<li><label>库位地址</label><input name="address" type="text" value="" /></li>
...
<script type="text/javascript">
	...
	function save(){
		...
		$.ajax({
		  type: 'POST',
		  async: false,
		  url: sysPath+'/warehouse/addWarehouse.do',
		  data: data,
		  dataType:"json",
	...
	$(document).ready(function() {
		...
		<%} else{%>			
			$('#span-departmentId').html(Common.departments.method.getName('<%=departmentId%>'));
		<%} %>
	});
</script>
</body>
