...
public class ExcelTemplate {
		...
		for(int i=gotoRow,len =sheets.getLastRowNum()+1;i<len;i++){
			...
			for(int j=0;j<cellnum;j++){
				...
				if(cell.getCellType()==XSSFCell.CELL_TYPE_NUMERIC){
					...
					map.put(keys[j], df.format(cell.getNumericCellValue()));
	...
	public List readRow(int sheetIndex,int gotoRow,String[] keys, Class clazz){
		Sheet sheets = this.wb.getSheetAt(sheetIndex);
		Row row= null;
		int cellnum =  0 ;
		...
		for(int i=gotoRow,len =sheets.getLastRowNum()+1;i<len;i++){
			...
			try {
				...
				for(int j=0;j<cellnum;j++){
					...
					Method setter = JavaBase.getSetter(clazz, JavaBase.getSetterByFieldName(keys[j]));
					if(setter == null){
						throw new RuntimeException("类中不存在指定此setter方法："+setter.getName());
					}
					...
					if(cell.getCellType()==XSSFCell.CELL_TYPE_NUMERIC){
						DecimalFormat df = new DecimalFormat("#");
						setter.invoke(obj, df.format(cell.getNumericCellValue()));
					}else if(cell.getCellType()==XSSFCell.CELL_TYPE_BOOLEAN){
						setter.invoke(obj, cell.getBooleanCellValue());
					}else if(cell.getCellType()==XSSFCell.CELL_TYPE_FORMULA){
						setter.invoke(obj, cell.getCellFormula());
					}else{
						setter.invoke(obj, StringUtils.trim(cell.getStringCellValue()));
					}
